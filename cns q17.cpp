...
// Function to perform the initial and final permutation
uint64_t permutation(uint64_t input, int *permTable, int tableLen) {
    uint64_t output = 0;
    for (int i = 0; i < tableLen; i++) {
        output <<= 1;
        output |= (input >> (64 - permTable[i])) & 0x01;
    }
    return output;
}

// Function to perform the DES f-function
uint32_t f(uint32_t R, uint64_t K) {
    uint64_t expandedR = 0;
    for (int i = 0; i < 48; i++) {
        expandedR <<= 1;
        expandedR |= (R >> (32 - E[i])) & 0x01;
    }
    ...
    uint32_t output = 0;
    for (int i = 0; i < 8; i++) {
        int row = ((expandedR >> (42 - 6 * i)) & 0x20) | ((expandedR >> (42 - 6 * i - 4)) & 0x01);
        int col = (expandedR >> (42 - 6 * i - 1)) & 0x0F;
        output <<= 4;
        output |= S[i][row][col];
    }
    ...
    for (int i = 0; i < 32; i++) {
        finalOutput <<= 1;
        finalOutput |= (output >> (32 - P[i])) & 0x01;
    }
    return finalOutput;
}

// Function to generate the 16 subkeys for DES decryption
void generateKeys(uint64_t key, uint64_t *subKeys) {
    // Initial key permutation tables
    int PC1[56] = {
        57, 49, 41, 33, 25, 17, 9, 
        1,  58, 50, 42, 34, 26, 18, 
        10, 2,  59, 51, 43, 35, 27, 
    ...
    uint64_t permutedKey = permutation(key, PC1, 56);
    ...
    for (int i = 0; i < 16; i++) {
        ...
        subKeys[15 - i] = permutation(combinedKey, PC2, 48);
    }
}

// DES Decryption Function
uint64_t desDecrypt(uint64_t ciphertext, uint64_t *subKeys) {
    ciphertext = permutation(ciphertext, IP, 64);
    ...
    return permutation(combined, IP_1, 64);
}

int main() {
    // Example 64-bit key and ciphertext
